...
public class Fox {
    ...
    public static void printMatrix(int[] matrix, int width, int height) {
        ...
        for(int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                res.append(matrix[i * height + j]).append("  ");
            }
            res.append("\n ");
        }
        res.append("___________\n ");
    ...
    public static void main(String[] args) {
        ...
        if(procRank == 0) {
            for(int i = 0; i < matSize; i++) {
                for(int j = 0; j < matSize; j++) {
                    matrixA[i * matSize + j] = rnd.nextInt(2);
                    matrixB[i * matSize + j] = rnd.nextInt(2);
                    matrixC[i * matSize + j] = 0;
                }
            }
        //   printMatrix(matrixA, matSize, matSize);

          //  printMatrix(matrixB, matSize, matSize);
        ...
        for (int i = 0; i < blockSize; i++) {
            ...
            for(int j = 0; j < gridSize * blockSize; j++)  resultRow[i * matSize + j] = subRowRes[j];
     ...
     //   if (gridCoords[1] == 0) {
       ...
       // }
        ...
        if(procRank == 0) {
           ...
           // printMatrix(matrixC, matSize, matSize);
        }
